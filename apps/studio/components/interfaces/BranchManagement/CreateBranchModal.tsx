...
export const CreateBranchModal = () => {
  ...
  const selectedAddons = Array.isArray(addons?.selected_addons) ? addons.selected_addons : []
  const computeAddon = selectedAddons.find((addon) => addon.type === 'compute_instance')
  const computeSize = !!computeAddon
    ? (computeAddon.variant.identifier.split('ci_')[1] as DesiredInstanceSize)
    : undefined
  const hasPitrEnabled =
    selectedAddons.find((addon) => addon.type === 'pitr') !== undefined
