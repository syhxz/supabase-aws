        ...
        <TableRow key={x.id}>
          ...
          <TableCell>
            <div className="flex gap-2 flex-wrap">
              {(() => {
                // Handle both string and array formats for events
                let events: string[] = []
                if (Array.isArray(x.events)) {
                  events = x.events
                } else if (typeof x.events === 'string') {
                  events = x.events.split(' OR ').map(e => e.trim()).filter(Boolean)
                } else if (x.events) {
                  console.warn('Unexpected events format:', x.events)
                  events = []
                }
