...
// Extended type to include database_name and database_user for self-hosted projects
type ProjectWithDatabase = OrgProject & { database_name?: string; database_user?: string }

export interface ProjectCardProps {
  slug?: string
  project: ProjectWithDatabase
  rewriteHref?: string
...
export const ProjectCard = ({
  ...
  const { name, ref: projectRef, database_name: databaseName, database_user: databaseUser } = project
            ...
            <div className="flex items-center gap-x-1.5">
              ...
              {databaseName && (
                <>
                  ...
                  <p className="text-xs !ml-2 text-foreground-light truncate" title={databaseName}>
                    {databaseName}
                  </p>
                  {databaseUser && (
                    <p className="text-xs text-foreground-lighter truncate" title={`User: ${databaseUser}`}>
                      @{databaseUser}
                    </p>
                  )}
                </>
