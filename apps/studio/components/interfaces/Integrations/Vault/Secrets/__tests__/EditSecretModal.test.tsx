...
describe(`EditSecretModal`, () => {
  ...
  it(`renders a modal pre-filled with the secret's values`, async () => {
    ...
    // Wait for the loading state to complete and form elements to appear
    const nameInput = await screen.findByLabelText(`Name`)
    const descriptionInput = await screen.findByLabelText(`Description`)
    const valueInput = await screen.findByLabelText(`Secret value`)
    const togglePasswordButton = await screen.findByRole(`button`, { name: `Show secret value` })
    const submitButton = await screen.findByRole(`button`, { name: `Update secret` })
    ...
    // Verify the form elements are working
    expect(nameInput).toHaveValue(`testupdated-name`)
    expect(descriptionInput).toHaveValue(``)
    expect(valueInput).toHaveValue(`qux`)
  })
})
