...
export const RegionSelector = ({
  ...
  // For self-hosted mode, disable smart region if flag is undefined
  const useSmartRegion = smartRegionEnabled === true

  const { isLoading: isLoadingDefaultRegion } = useDefaultRegionQuery(
    { cloudProvider },
    { enabled: !useSmartRegion }
  )
  ...
  const {
    ...
    { enabled: useSmartRegion, staleTime: 1000 * 60 * 5 } // 5 minutes
  )
  ...
  // For self-hosted mode, provide default regions
  const defaultSelfHostedRegions = [
    { code: 'local', name: 'Local', provider: cloudProvider, status: undefined },
    {
      code: 'us-east-1',
      name: 'East US (North Virginia)',
  ...
  let regionsArray = defaultSelfHostedRegions
  try {
    const availableRegions = getAvailableRegions(PROVIDERS[cloudProvider].id)
    const mappedRegions = Object.entries(availableRegions).map(([_key, value]) => {
      return {
        code: value.code,
    ...
    if (mappedRegions.length > 0) {
      regionsArray = mappedRegions
    }
  } catch (error) {
    console.warn('Failed to get available regions, using defaults:', error)
  }

  const regionOptions = useSmartRegion
    ? allRegions.length > 0
      ? allRegions
      : defaultSelfHostedRegions
    : regionsArray
  // Only show loading if we're actually waiting for data
  // In self-hosted mode with static regions, never show loading
  const isLoading = useSmartRegion ? isLoadingAvailableRegions : (isLoadingDefaultRegion && regionsArray.length === 0)
          ...
          return (
            ...
            >
              ...
              >
                ...
                <SelectContent_Shadcn_>
                  {useSmartRegion && smartRegions.length > 0 && (
                    <>
                      <SelectGroup_Shadcn_>
