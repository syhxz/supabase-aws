...
import { getDatabaseDisplayLabel } from 'lib/database-display-utils'
...
export const PrimaryNode = ({ data }: NodeProps<PrimaryNodeData>) => {
  ...
  const { id, provider, region, computeSize, numReplicas, numRegions, hasLoadBalancer } = data
  ...
  // Get the proper database type label using enhanced identification logic
  // Ensure primary databases are always labeled as "Primary Database"
  const databaseTypeLabel = getDatabaseDisplayLabel(projectRef || '', id, databases || [])
  
  // Additional safety check to ensure primary databases show correct label
  const finalLabel = (id === projectRef || databaseTypeLabel === 'Primary Database') 
    ? 'Primary Database' 
    : databaseTypeLabel
      ...
      <div className="flex flex-col rounded bg-surface-100 border border-default">
        ...
        >
          <div className="flex gap-x-3">
            ...
            <div className="flex flex-col gap-y-0.5">
              <p className="text-sm">{finalLabel}</p>
...
export const ReplicaNode = ({ data }: NodeProps<ReplicaNodeData>) => {
  ...
  // Ensure correct labeling for database type
  const finalDatabaseLabel = (id === projectRef || databaseTypeLabel === 'Primary Database') 
    ? 'Primary Database' 
    : databaseTypeLabel

  const [, setShowConnect] = useQueryState('showConnect', parseAsBoolean.withDefault(false))

  const { data: databaseStatuses } = useReadReplicasStatusesQuery({ projectRef })
          ...
          <div className="flex flex-col gap-y-0.5">
            <div className="flex items-center gap-x-2">
              <p className="text-sm truncate">
                {finalDatabaseLabel} {id.length > 0 && `(ID: ${formatDatabaseID(id)})`}
              </p>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-40" side="bottom" align="end">
            ...
            <DropdownMenuItem
              ...
              disabled={status !== REPLICA_STATUS.ACTIVE_HEALTHY}
            >
              <Link href={`/project/${projectRef}/observability/database?db=${id}&chart=replication-lag`}>
