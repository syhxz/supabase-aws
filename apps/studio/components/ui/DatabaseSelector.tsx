...
import { getDatabaseDisplayLabel, isDatabasePrimary } from 'lib/database-display-utils'
              ...
              <>
                <span className="capitalize">
                  {isLoading 
                    ? 'Primary Database'
                    : (() => {
                        const label = getDatabaseDisplayLabel(projectRef || '', selectedDatabaseId || '', databases)
                        // Ensure primary databases always show "Primary Database"
                        return (selectedDatabaseId === projectRef || label === 'Primary Database') 
                          ? 'Primary Database' 
                          : label
                      })()
                  }
                </span>{' '}
                {isSuccess && !isDatabasePrimary(projectRef || '', selectedDatabaseId || '', databases) && (
                  <span>
                        ...
                        <TooltipTrigger asChild>
                          <div className="px-2 py-1.5 w-full flex items-center justify-between">
                            <p className="text-xs text-foreground-lighter">
                              {(() => {
                                const label = getDatabaseDisplayLabel(projectRef || '', database.identifier || '', databases)
                                return (database.identifier === projectRef || label === 'Primary Database') 
                                  ? 'Primary Database' 
                                  : label
                              })()} ({region} - {id})
                            </p>
                    ...
                    >
                      <div className="w-full flex items-center justify-between">
                        <p>
                          {(() => {
                            const label = getDatabaseDisplayLabel(projectRef || '', database.identifier || '', databases)
                            const finalLabel = (database.identifier === projectRef || label === 'Primary Database') 
                              ? 'Primary Database' 
                              : label
                            
                            return isDatabasePrimary(projectRef || '', database.identifier || '', databases)
                              ? finalLabel
