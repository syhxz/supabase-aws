...
export type ProjectCreateVariables = {
  ...
  dbPass: string
  // Note: databaseName and databaseUser removed - always auto-generated by API
  dbRegion?: string
...
export async function createProject({
  ...
  const body: CreateProjectBody = {
    ...
    name,
    db_pass: dbPass,
    // Note: database_name and database_user removed - always auto-generated by API
    db_region: dbRegion,
  ...
  // Use the create endpoint for project creation
  const endpoint = `/platform/projects/create`
  ...
  return useMutation<ProjectCreateData, ResponseError, ProjectCreateVariables>({
    ...
    async onError(data, variables, context) {
      if (onError === undefined) {
        // Check if this is a credential generation error with enhanced details
        const errorDetails = (data as any)?.error?.details
        if (errorDetails?.userAction && errorDetails?.suggestions) {
          // Show enhanced error message for credential generation failures
          toast.error(
            `Failed to create new project: ${data.message}`,
            {
              description: errorDetails.userAction,
