...
exports.getFullTextSearchService = getFullTextSearchService;
const project_database_client_1 = require("./project-database-client");
/**
...
class FullTextSearchService {
     ...
     */
    parseFullTextSearchOperators(query) {
        const ftsFilters = [];
     ...
     * Requirements: 4.1, 4.2, 4.3, 4.4
     */
    isFullTextSearchOperator(operator) {
     ...
     * Requirements: 4.1, 4.2, 4.3, 4.4, 4.5
     */
    parseFullTextSearchFilter(column, operator, value, configParts) {
        try {
     ...
     * Requirements: 4.5
     */
    isValidTextSearchConfig(config) {
     ...
     * Requirements: 4.1, 4.2, 4.3, 4.4, 4.5
     */
    buildFullTextSearchWhereClause(filters) {
        if (filters.length === 0) {
     ...
     * Requirements: 4.1, 4.2, 4.3, 4.4, 4.5
     */
    buildSingleFullTextSearchCondition(filter, paramIndex) {
     ...
     * Requirements: 4.1, 4.2, 4.3, 4.4, 4.5
     */
    async executeFullTextSearchQuery(context, tableName, ftsFilters, selectColumns, orderBy, limit, offset, includeRanking) {
        ...
        try {
            ...
            // Execute the query
            const client = (0, project_database_client_1.createProjectDatabaseClient)(context);
     ...
     * Requirements: 4.5
     */
    async checkForUnindexedColumns(context, tableName, filters) {
        const warnings = [];
        try {
            const client = (0, project_database_client_1.createProjectDatabaseClient)(context);
            for (const filter of filters) {
        ...
        `;
                const indexResult = await client.query(indexQuery, [tableName, filter.column]);
                if (indexResult.rowCount === 0) {
                    warnings.push(`Column "${filter.column}" does not have a full-text search index. ` +
     ...
     */
    async getAvailableConfigurations(context) {
        try {
            const client = (0, project_database_client_1.createProjectDatabaseClient)(context);
            const query = `
     ...
     */
    getConfigurationDescription(configName) {
        const descriptions = {
