...
exports.validateProjectCredentials = validateProjectCredentials;
exports.validateCredentialFormat = validateCredentialFormat;
exports.generateValidationErrorReport = generateValidationErrorReport;
 ...
 */
async function validateProjectCredentials(credentials, options = {}) {
    const errorHandler = (0, credential_error_handling_1.getCredentialErrorHandler)();
    return errorHandler.executeWithErrorHandling(async () => validateProjectCredentialsSync(credentials, options), {
 ...
 */
function validateProjectCredentialsSync(credentials, options = {}) {
 ...
 */
function validateCredentialFormat(user, passwordHash) {
    const errors = [];
    const warnings = [];
 ...
 */
function generateValidationErrorReport(validationResult, context = {}) {
 ...
 */
function logValidationFailure(validationResult, context = {}) {
...
function calculatePasswordStrength(password, config) {
    ...
    if (/(.)\1{2,}/.test(password))
        patternPenalty += 5; // Repeated characters
    if (hasSequentialChars(password))
        patternPenalty += 5; // Sequential characters
    if (/^(.+)\1+$/.test(password))
        patternPenalty += 5; // Repeated patterns
 ...
 */
function areCredentialsTooSimilar(username, password) {
 ...
 */
function containsSqlInjectionPatterns(username) {
 ...
 */
function isLikelyPlaintextPassword(password) {
