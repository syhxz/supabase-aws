...
exports.verifyCrossProjectAccessDenial = verifyCrossProjectAccessDenial;
exports.verifyUserPermissions = verifyUserPermissions;
exports.logUserAccess = logUserAccess;
exports.runIsolationVerification = runIsolationVerification;
exports.generateIsolationVerificationScript = generateIsolationVerificationScript;
 ...
 */
async function verifyCrossProjectAccessDenial(username, targetDatabase) {
    try {
        (0, database_user_manager_1.validateUsername)(username);
    }
    catch (error) {
        if (error instanceof database_user_manager_1.DatabaseUserError) {
            return { data: undefined, error };
        }
        return {
            data: undefined,
            error: new database_user_manager_1.DatabaseUserError(database_user_manager_1.DatabaseUserErrorCode.UNKNOWN_ERROR, error instanceof Error ? error.message : 'Validation failed')
        };
    }
    // Check if user exists
    const { userExists } = await Promise.resolve().then(() => __importStar(require('./database-user-manager')));
    ...
    if (!existsResult.data) {
        return {
            data: undefined,
            error: new database_user_manager_1.DatabaseUserError(database_user_manager_1.DatabaseUserErrorCode.INVALID_USERNAME, `User "${username}" does not exist`, { username })
        };
    }
 ...
 */
async function verifyUserPermissions(username, assignedDatabase) {
    try {
        (0, database_user_manager_1.validateUsername)(username);
    }
    catch (error) {
    ...
    const permissionsResult = await (0, database_user_manager_1.getUserPermissions)(username);
 ...
 */
async function logUserAccess(auditLog) {
...
async function runIsolationVerification() {
    ...
    try {
      ...
      ORDER BY usename
    `;
        const usersResult = await (0, query_1.executeQuery)({
            query: usersQuery,
            readOnly: true,
        });
      ...
      ORDER BY datname
    `;
        const databasesResult = await (0, query_1.executeQuery)({
        ...
        return {
            data: undefined,
            error: new database_user_manager_1.DatabaseUserError(database_user_manager_1.DatabaseUserErrorCode.UNKNOWN_ERROR, error instanceof Error ? error.message : 'Isolation verification failed')
        };
    }
}
/**
 ...
 */
function generateIsolationVerificationScript() {
    return `
