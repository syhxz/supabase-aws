...
exports.SSLErrorHandler = void 0;
exports.getSSLErrorHandler = getSSLErrorHandler;
/**
...
class SSLErrorHandler {
     ...
     * Requirements: 12.4
     */
    generateErrorResponse(error, projectRef, sslMode) {
        const suggestions = [];
    ...
    validateSSLConfig(sslConfig) {
        ...
        if (typeof sslConfig === 'string') {
            const validModes = ['require', 'prefer', 'allow', 'disable', 'verify-ca', 'verify-full'];
            if (!validModes.includes(sslConfig)) {
                result.isValid = false;
                result.errors.push(`Invalid SSL mode: ${sslConfig}. Valid modes are: ${validModes.join(', ')}`);
            }
            else {
                result.sslEnabled = sslConfig !== 'disable';
                result.sslMode = sslConfig;
            }
            return result;
        }
     ...
     */
    getSSLRecommendations(environment) {
        switch (environment) {
            case 'production':
                return {
...
exports.SSLErrorHandler = SSLErrorHandler;
/**
 * Factory function to get the SSL error handler
 */
function getSSLErrorHandler() {
