...
export async function retrieveAnalyticsData({
  ...
  try {
    console.log(`[Analytics API] Making request to: ${url.toString()}`)
    ...
    if (!response.ok) {
      console.error(
        `[Analytics API] Request failed: ${url.toString()} - Status: ${response.status} ${response.statusText}`
      )
    ...
    console.error(`[Analytics API] Request error: ${url.toString()} - ${error instanceof Error ? error.message : 'Unknown error'}`)
