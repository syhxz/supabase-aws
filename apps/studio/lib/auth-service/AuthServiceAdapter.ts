...
export class AuthServiceAdapter {
  ...
  constructor() {
    // Validate issuer configuration on initialization
    this.validateIssuerConfiguration()
  }

  /**
   * Validate that issuer configuration is properly set
   * 
   * Checks if SUPABASE_URL or API_EXTERNAL_URL environment variables are configured.
   * Logs a warning if neither is set, or confirms the issuer value if properly configured.
   * 
   * Requirements: 3.1, 3.2, 3.3
   */
  private validateIssuerConfiguration(): void {
    const issuer = process.env.SUPABASE_URL || process.env.API_EXTERNAL_URL
    
    if (!issuer) {
      console.warn('[AuthServiceAdapter] Warning: Neither SUPABASE_URL nor API_EXTERNAL_URL is configured. JWT issuer claim will be undefined.')
    } else {
      console.log('[AuthServiceAdapter] JWT issuer configured:', issuer)
    }
  }
  ...
  private async createSession(
    ...
    // The issuer claim (iss) is required for JWT verification to succeed.
    // It identifies the principal that issued the JWT and must match the
    // validIssuers configuration in the verification logic.
    // Priority: SUPABASE_URL (preferred) > API_EXTERNAL_URL (fallback)
    const issuer = process.env.SUPABASE_URL || process.env.API_EXTERNAL_URL
    const signOptions: jwt.SignOptions = {
      expiresIn: this.JWT_EXPIRY,
    }
    
    // Only include issuer if it's defined (jsonwebtoken requires it to be a string or omitted)
    if (issuer) {
      signOptions.issuer = issuer
    }
    
    // Debug log: JWT generation with issuer value
    // Requirements: 6.1, 6.4
    console.debug('[AuthServiceAdapter] Generating JWT', {
      userId: user.id,
      projectRef,
      issuer: issuer || 'undefined',
      role: user.is_super_admin ? 'admin' : 'authenticated',
      sessionId,
      // Note: JWT secret and token are NOT logged for security
    })
