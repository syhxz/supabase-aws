...
async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // MCP server doesn't need a specific project ID for self-hosted mode
    // The platform operations handle project routing internally
    const server = createSupabaseMcpServer({
      platform,
      projectId: 'mcp-server', // Use a non-conflicting identifier
      features,
      readOnly: read_only,
    })
