...
const handleGet = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (result.error) {
    ...
    if (userId) {
      // Filter to only include projects directly owned by the user
      // This ensures proper user isolation - users can only see their own projects
      projectsData = projectsData.filter(project => {
        return project.owner_user_id === userId
      })
      
      console.log('[Org Projects API] User owns', projectsData.length, 'projects directly')
      console.log('[Org Projects API] After user isolation filtering:', projectsData.length, 'projects accessible to user')
    } else {
