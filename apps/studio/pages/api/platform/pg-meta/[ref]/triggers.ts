...
const handleGetAll = async (req: NextApiRequest, res: NextApiResponse) => {
  if (!IS_PLATFORM) {
    ...
    try {
      ...
      // Post-process the results to clean up the events array
      const processedRows = result.rows.map(row => ({
        ...row,
        events: row.events ? row.events.filter(Boolean) : []
      }))
      
      console.log(`[API] Found ${processedRows.length} triggers for project: ${projectRef}`)
