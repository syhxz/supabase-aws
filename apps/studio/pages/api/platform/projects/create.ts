 ...
 * 
 * Authentication:
 * - Accepts GoTrue JWT tokens without project_ref claim
 * - Only validates user identity (sub, email) for project creation
 * - Project-specific JWTs are not required since the project doesn't exist yet
 * 
 * Features:
...
const handlePost = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (isUserIsolationEnabled()) {
      // For project creation, we accept GoTrue JWT without project_ref claim
      // since the project doesn't exist yet. We only need to verify user identity (sub, email).
