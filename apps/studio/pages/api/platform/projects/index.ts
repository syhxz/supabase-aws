...
const handleGet = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (userIsolation) {
    ...
    // Get all projects to filter by ownership and organization membership
    ...
    // Filter projects based on user access (owner only for now)
    if (result.data && result.data.length > 0) {
      // Filter to include only projects directly owned by the user
      ...
      console.log('[Projects API] User owns', result.data.length, 'out of', originalCount, 'total projects')
    }
  } else {
  ...
  if (result.error) {
    ...
    allProjects = result.data.map((project) => ({
      ...
      connectionString: generateDisplayConnectionString({
        ...
        user: project.database_user || undefined,
        password: project.database_password_hash || undefined,
        readOnly: false,
      }),
      databases: [
        {
