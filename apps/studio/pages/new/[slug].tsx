...
import { 
  CredentialGenerationStatus, 
  useCredentialGenerationStatus,
  type CredentialGenerationStatus as CredentialGenerationStatusType,
} from 'components/interfaces/ProjectCreation/CredentialGenerationStatus'
...
const Wizard: NextPageWithLayout = () => {
  ...
  // Credential generation status tracking
  const credentialStatus = useCredentialGenerationStatus()
  ...
  const { instanceSize: watchedInstanceSize, cloudProvider, dbRegion, organization, projectName: watchedProjectName } = form.watch()
  ...
  const { data: orgProjectsFromApi, error: orgProjectsError } = useOrgProjectsInfiniteQuery({ 
  ...
  // Handle project loading errors gracefully - don't break the UI
  if (orgProjectsError) {
    console.error('Failed to load organization projects:', orgProjectsError)
  }
  ...
  const {
    ...
    onSuccess: (res) => {
      credentialStatus.updateStatus('generation-complete')
      track(
    ...
    onError: (error) => {
      // Check if this is a credential generation error
      const errorDetails = (error as any)?.error?.details
      if (errorDetails?.code && errorDetails.code.includes('GENERATION')) {
        credentialStatus.setGenerationError(
          errorDetails.code,
          error.message,
          errorDetails,
          errorDetails.attempts || 1
        )
      } else {
        credentialStatus.updateStatus('error')
      }
    },
  })
  ...
  const onSubmit = async (values: z.infer<typeof FormSchema>) => {
    ...
    // Start credential generation status tracking
    credentialStatus.updateStatus('generating-database-name')
              ...
              <div className="divide-y divide-border-muted">
                ...
                {canCreateProject && (
                  <>
                    ...
                    {/* Show credential generation status during project creation */}
                    {(isCreatingNewProject || credentialStatus.status !== 'idle') && (
                      <Panel.Content>
                        <CredentialGenerationStatus
                          ...
                          onRetry={() => {
                            ...
                            form.handleSubmit((values) => {
                              ...
                              createProject({
                                ...
                                dbPricingTierId: 'tier_free',
                                dbInstanceSize: isFreePlan ? undefined : (values.instanceSize as DesiredInstanceSize),
                                dataApiExposedSchemas: !values.dataApi ? [] : undefined,
                                dataApiUseApiSchema: !values.dataApi ? false : values.useApiSchema,
                                ...(smartRegionEnabled ? {} : { dbRegion: values.dbRegion }),
                              })
                            })(formValues)
                          }}
                          className="border-b border-border-muted pb-6"
                        />
                      </Panel.Content>
                    )}
