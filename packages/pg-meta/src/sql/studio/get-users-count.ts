...
export const getUsersCountSQL = ({
  ...
  if (optimizedSearchMode) {
    if (column === 'email') {
      ...
      conditions.push(
        `lower(email) >= ${lowerBound}${upperBound ? ` and lower(email) < ${upperBound}` : ''}`
      )
      conditions.push(`instance_id = '00000000-0000-0000-0000-000000000000'::uuid`)
  ...
  if (forceExactCount) {
    return `select (${baseQueryCount}${whereClause}) as count, false as is_estimate;`
  } else {
